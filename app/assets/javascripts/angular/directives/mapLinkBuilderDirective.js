    ...
    return {
        ...
        link: function(scope, elm) {
          ...
          if (scope.exam.location.lat && scope.exam.location.lon) {
            element = $compile('<a href="https://maps.google.com/maps?q={{exam.location.lat}},{{exam.location.lon}}+({{exam.location.building_link_name}})" title="{{exam.location.building_tooltip_name}}">{{exam.location.raw_location}} <i class="cc-icon icon-map-marker"></i></a>')(scope);
          } else {
