module OecLegacy
  class CoursesGroup
    ...
    def initialize(departments, dest_dir, keep_csv_files = false, debug_mode = false)
      @dest_dir = dest_dir
      @csv_per_dept = {}
      registry = OecLegacy::DepartmentRegistry.new departments
      registry.each do |dept_name|
        courses = OecLegacy::Courses.new(dept_name, dest_dir)
        courses.export
      ...
      if registry.include? biology
        ...
        post_processor = OecLegacy::BiologyPostProcessor.new(dest_dir, dest_dir, debug_mode)
      ...
      @csv_per_dept.each do |dept_name, csv_file|
        campus_data = []
        OecLegacy::Csv.read(csv_file).each_with_index do |row, index|
          campus_data << OecLegacy::RowConverter.new(row).hashed_row if index > 0 && row.length > 0
        end
