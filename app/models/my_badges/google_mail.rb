module MyBadges
  class GoogleMail
    ...
    def internal_fetch_counts(params = {})
      google_proxy = Google::GoogleMailListProxy.new(user_id: @uid)
      ...
      if google_mail_results && google_mail_results.response
        ...
        begin
          ...
          Rails.logger.fatal "Error parsing XML output for Google::GoogleMailListProxy: #{e}"
          nokogiri_xml = nil
        end
    ...
    def get_count(nokogiri_xml)
      begin
        ...
        Rails.logger.warn "#{self.class.name}: Error parsing XML output for unread counts from Google::GoogleMailListProxy: #{e}"
        return 0
      end
    end
    ...
    def get_items(nokogiri_xml)
      ...
      begin
        ...
        Rails.logger.fatal "#{self.class.name} Error parsing XML output for mail items from Google::GoogleMailListProxy: #{e}"
