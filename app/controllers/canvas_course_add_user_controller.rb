class CanvasCourseAddUserController < ApplicationController
  ...
  def search_users
    users_found = Canvas::CanvasCourseAddUser.search_users(params['search_text'], params['search_type'])
    render json: { users: users_found }.to_json
  end

  # GET /api/academics/canvas/course_add_user/course_sections.json
  def course_sections
    sections_list = Canvas::CanvasCourseAddUser.course_sections_list(@canvas_course_id)
    render json: { course_sections: sections_list }.to_json
  end

  # POST /api/academics/canvas/course_add_user/add_user.json
  def add_user
    Canvas::CanvasCourseAddUser.add_user_to_course_section(params[:ldap_user_id], params[:role_id], params[:section_id])
