class MyEventsController < ApplicationController
  ...
  def create
    ...
    if input.present?
      ...
      response = GoogleEventsInsertProxy.new(user_id: session[:user_id]).insert_event(input.stringify_keys)

      result = response.data || {}
      respond_with(response.data) do |format|
  ...
  def check_google_access
    return error_response unless GoogleProxy.access_granted?(session[:user_id])
    return error_response if (is_acting_as_nonfake_user? && !GoogleProxy.allow_pseudo_user?)
  end
  ...
  def sanitize_input!(params)
    ...
    %w(start end).each do |key|
      if params[key] && params[key][:epoch]
        ...
        result[key.to_sym] = { dateTime: date_time.new_offset('-7:00').rfc3339(2) } if date_time.present?
      end
    end
    ...
    result.merge({calendarId: "primary"})
  end
