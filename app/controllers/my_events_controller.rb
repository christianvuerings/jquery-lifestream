class MyEventsController < ApplicationController
  ...
  def create
    ...
    if input.present?
      ...
      response = Google::GoogleEventsInsertProxy.new(user_id: session[:user_id]).insert_event(input.stringify_keys)

      result = response.data || {}
      if result.blank?
        logger.warn "Google::GoogleEventsInsertProxy.insert_event response for user #{session[:user_id]} should not be blank.
  ...
  def check_google_access
    return error_response unless Google::GoogleProxy.access_granted?(session[:user_id])
    return error_response if (is_acting_as_nonfake_user? && !Google::GoogleProxy.allow_pseudo_user?)
  end
