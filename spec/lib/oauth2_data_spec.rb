...
describe Oauth2Data do
  ...
  before do
    ...
    @fake_canvas_user_profile = Canvas::CanvasUserProfileProxy.new(fake: true, user_id: 300846)
  end
  ...
  it "should fail updating canvas email on a non-existant Canvas account" do
    Canvas::CanvasUserProfileProxy.any_instance.stub(:user_profile).and_return(@canvas_404)
    Oauth2Data.new_or_update(@random_id, Canvas::CanvasProxy::APP_ID, "new-token",
                             "some-token", 1)
  ...
  it "should successfully update a canvas email " do
    Canvas::CanvasUserProfileProxy.stub(:new).and_return(@fake_canvas_user_profile)
    Oauth2Data.new_or_update(@random_id, Canvas::CanvasProxy::APP_ID, "new-token",
                             "some-token", 1)
    Oauth2Data.get_canvas_email(@random_id).blank?.should be_true
    Oauth2Data.update_canvas_email!(@random_id)
