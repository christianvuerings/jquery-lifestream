...
describe "HotPlate" do
  ...
  it "should warm the cache of some users who have visited recently" do
    ...
    Calcentral::Messaging.should_receive(:publish).with('/queues/hot_plate', "1234").exactly(1).times
    Calcentral::Messaging.should_receive(:publish).with('/queues/hot_plate', "5678").exactly(1).times
    @plate.warm
  end
