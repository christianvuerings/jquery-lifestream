...
describe 'PopulateLectureInstanceEnrollment' do
  before :each do
    ...
    if ENV['RAILS_ENV'] == 'test'
      csv << '1,SEC:2013-C-7309,1,COURSE:BIO:1A:2013-B,BIO 1A:Advanced Biology for poets,active,,,1,ACCT:BIO'.parse_csv
    elsif ENV['RAILS_ENV'] == 'testext'
      csv << '1,SEC:2013-D-54282,1,COURSE:MATH:185:2013-D,MATH 185:Basic Math for botanists,active,,,1,ACCT:MATH'.parse_csv
    end
    csv << '3,,5,,Not hooked to enrollments,active,,,1,ACCT:BIO'.parse_csv
  ...
  it 'should have a successful import' do
    ...
    if ENV['RAILS_ENV'] == 'test'
      ...
      result[:added_enrollments]["SEC:2013-C-7309"].should == 1
    elsif ENV['RAILS_ENV'] == 'testext'
      result[:added_students].should >= 1
