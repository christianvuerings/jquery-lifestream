...
describe CanvasUserProvision do

  describe "#csv_filename_prefix" do
    before do
      SecureRandom.stub(:hex).and_return('f76d1b860dcc152c')
      static_time = DateTime.strptime('2013-11-05T12:05:05+07:00', '%Y-%m-%dT%H:%M:%S%z')
      DateTime.stub(:now).and_return(static_time)
    end
    ...
    it "caches and returns previously generated filename prefix" do
      ...
      SecureRandom.stub(:hex).and_return('d65c0a759cbb041b')
      expect(subject.csv_filename_prefix).to eq "tmp/canvas/user_provision-2013-11-05-f76d1b860dcc152c"
    end
  end
  ...
  describe "#import_users" do
    before do
      subject.stub(:csv_filename_prefix).and_return("tmp/canvas/user_provision-2013-11-05-f76d1b860dcc152c")
      user_definitions = [
      ...
      subject.stub(:accumulate_user_data).with(['1234','1235'], []).and_return(user_definitions)
    end
    ...
    it "passes transformed UID list to Canvas::CanvasSisImportProxy#import_users as CSV definitions" do
      Canvas::CanvasSisImportProxy.any_instance.should_receive(:import_users).with("tmp/canvas/user_provision-2013-11-05-f76d1b860dcc152c-users.csv").and_return(true)
      result = subject.import_users(['1234','1235'])
      expect(result).to be_true
    end

    it "raises exception if user import failed" do
      Canvas::CanvasSisImportProxy.any_instance.should_receive(:import_users).with("tmp/canvas/user_provision-2013-11-05-f76d1b860dcc152c-users.csv").and_return(nil)
