describe OecLegacy::CommandLine do
  ...
  it 'should pay attention ENV variables' do
    ...
    ENV['dest'] = dest_path
    ENV['options'] = 'd , y'
    cmd_line = OecLegacy::CommandLine.new
  ...
  it 'should parse comma-separated values of "options" arg' do
    ENV['options'] = ' Y '
    expect(OecLegacy::CommandLine.new.is_limited_diff).to be_truthy
    expect(OecLegacy::CommandLine.new.is_debug_mode).to_not be_truthy
  end
  ...
  it 'should reference logic of department-registry to pick up BIOLOGY-related relationships' do
    ...
    departments = OecLegacy::CommandLine.new.departments
  ...
  it 'should pull all OEC departments when none specified' do
    ENV['departments'] = nil
    OecLegacy::CommandLine.new.departments.should be_empty
  end
