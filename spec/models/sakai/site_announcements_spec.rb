...
describe Sakai::SiteAnnouncements do
  ...
  it "should return something even when there is no announcement tool in the site" do
    client = Sakai::SiteAnnouncements.new({site_id: 'no-such-site'})
  ...
  it "should find seeded attachments", :if => Sakai::SakaiData.test_data? do
    client = Sakai::SiteAnnouncements.new({site_id: @site_with_ten_current})
    announcements = client.get_announcements
    attachments = []
    announcements.each do |announcement|
  ...
  it "should return only site-wide announcements by default", :if => Sakai::SakaiData.test_data? do
    client = Sakai::SiteAnnouncements.new({site_id: @site_with_ten_current})
  ...
  it "should not receive draft, lapsed, or unreleased announcements", :if => Sakai::SakaiData.test_data? do
