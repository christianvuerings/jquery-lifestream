...
describe CalLinkMembershipsProxy do

  it "should get the fake membership feed from CalLink" do
    client = CalLinkMembershipsProxy.new({:user_id => "300846", :fake => true})
  ...
  it "should get the real membership feed from CalLink", :testext => true do
    client = CalLinkMembershipsProxy.new({:user_id => "300846", :fake => false})
    data = client.get_memberships
