...
describe Financials::Proxy do
  ...
  let(:live_oski_financials) { Financials::Proxy.new({user_id: "61889"}).get }
  let(:fake_tammi_financials) { Financials::Proxy.new({user_id: "300940", fake: true}).get }
  let(:fake_oski_financials) { Financials::Proxy.new({user_id: "61889", fake: true}).get }
  let(:non_student_financials) { Financials::Proxy.new({user_id: "212377"}).get }
