  ...
  context "when requesting course from canvas" do
    context "if course exists in canvas" do
      ...
      it "uses cache by default" do
        Canvas::SisCourse.should_receive(:fetch_from_cache).and_return({:cached => 'hash'})
        course = subject.course
      ...
      it "bypasses cache when cache option is false" do
        Canvas::SisCourse.should_not_receive(:fetch_from_cache)
    ...
    context "if course does not exist in canvas" do
      before { Canvas::SisCourse.any_instance.should_receive(:request_uncached).and_return(nil) }
      it "returns nil" do
        course = subject.course
