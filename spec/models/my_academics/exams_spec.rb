...
describe "MyAcademics::Exams" do
  ...
  it "should format fake Bearfacts exam data correctly" do
    proxy = Bearfacts::BearfactsExamsProxy.new({:user_id => "61889", :fake => true})
    Bearfacts::BearfactsExamsProxy.stub(:new).and_return(proxy)
  ...
  it "should properly handle a student with an exam in an unparseable room" do
    proxy = Bearfacts::BearfactsExamsProxy.new({:user_id => "865826", :fake => true})
    Bearfacts::BearfactsExamsProxy.stub(:new).and_return(proxy)
  ...
  it "should handle badly formatted BearfactsExamProxy XML responses" do
    proxy = Bearfacts::BearfactsExamsProxy.new({:user_id => "865826", :fake => true})
    Bearfacts::BearfactsExamsProxy.stub(:new).and_return(proxy)
    Bearfacts::BearfactsExamsProxy.any_instance.stub(:get).and_return({body: 'gobbly gook', status_code: 200})
