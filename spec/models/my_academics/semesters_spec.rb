...
describe "MyAcademics::Semesters", :if => CampusOracle::SakaiData.test_data? do
  let!(:oski_schedule_proxy) { CampusOracle::CampusUserCoursesProxy.new({:fake => true}) }
  ...
  context "should get properly formatted data from fake Oracle MV" do
    before(:each) do
      ...
      CampusOracle::CampusUserCoursesProxy.stub(:new).and_return(oski_schedule_proxy)
    end
  ...
  context "grading options" do
    before(:each) do
      Settings.sakai_proxy.academic_terms.stub(:student).and_return(nil)
      Settings.sakai_proxy.academic_terms.stub(:instructor).and_return(nil)
      @oski_campus_courses = CampusOracle::CampusUserCoursesProxy.new({:fake => true}).get_all_campus_courses
    end
    ...
    context "P/NP grading option (course default)" do
      before(:each) do
        ...
        CampusOracle::CampusUserCoursesProxy.any_instance.stub(:get_all_campus_courses).and_return(@oski_campus_courses)
      end
