...
describe CampusOracle::CampusData do
  ...
  context "looking up students" do
    ...
    unless CampusOracle::OecData.test_data?
      it { subject[0]["ldap_uid"].should_not be_nil }
    end
  end

  context "looking up instructors" do
    subject { CampusOracle::OecData.get_all_instructors(["11684"]) }
    it { should_not be_nil }
    unless CampusOracle::OecData.test_data?
      it { subject[0]["ldap_uid"].should_not be_nil }
    end
  ...
  context "looking up courses", :testext => true do
    subject { CampusOracle::OecData.get_all_courses }
    it { should_not be_nil }
    unless CampusOracle::OecData.test_data?
  ...
  context "looking up course_instructors" do
    subject { CampusOracle::OecData.get_all_course_instructors(["11684"]) }
    it { should_not be_nil }
    unless CampusOracle::OecData.test_data?
      it { subject[0]["ldap_uid"].should_not be_nil }
    end
  ...
  context "looking up course_students" do
    subject { CampusOracle::OecData.get_all_course_students(["11684"]) }
    it { should_not be_nil }
    unless CampusOracle::OecData.test_data?
      it { subject[0]["ldap_uid"].should_not be_nil }
    end
