...
describe CampusOracle::CampusData do
  ...
  it "should find Oliver" do
    data = CampusOracle::CampusData.get_person_attributes(2040)
    data['first_name'].should == "Oliver"
    if CampusOracle::CampusData.test_data?
  ...
  it "should find a user who has a bunch of blocks" do
    data = CampusOracle::CampusData.get_person_attributes(300847)
    if CampusOracle::CampusData.test_data?
  ...
  it "should find the most currently available student data between terms" do
    CampusOracle::CampusData.stub(:current_year).and_return(2525)
    data = CampusOracle::CampusData.get_person_attributes(300846)
    if CampusOracle::CampusData.test_data?
  ...
  it "should find Stu TestB's registration status" do
    data = CampusOracle::CampusData.get_reg_status(300846)
    if CampusOracle::CampusData.test_data?
  ...
  it "should return nil from get_reg_status if an existing user has no reg status" do
    data = CampusOracle::CampusData.get_reg_status("2040")
    data.should be_nil
  end

  it "should return nil from get_reg_status if the user does not exist" do
    data = CampusOracle::CampusData.get_reg_status("0")
    data.should be_nil
  end

  it "should find some students in Biology 1a" do
    students = CampusOracle::CampusData.get_enrolled_students("7309", "2013", "D")
  ...
  it "should find a course" do
    course = CampusOracle::CampusData.get_course_from_section("07366", "2013", "B")
  ...
  it "should find sections from CCNs" do
    courses = CampusOracle::CampusData.get_sections_from_ccns("2013", "D", ["7309", "07366", "919191", "16171"])
  ...
  it "should find where a person is enrolled, with grades where available" do
    sections = CampusOracle::CampusData.get_enrolled_sections('300939')
    sections.should_not be_nil
    transcripts = CampusOracle::CampusData.get_transcript_grades('300939')
  ...
  it "should be able to limit enrollment queries" do
    sections = CampusOracle::CampusData.get_enrolled_sections('300939', @current_terms)
    sections.should_not be_nil
  ...
  it "should find where a person is teaching" do
    sections = CampusOracle::CampusData.get_instructing_sections('238382')
    sections.should_not be_nil
    sections.length.should == 4 if CampusOracle::CampusData.test_data?
  end
  ...
  it "should be able to limit teaching assignment queries" do
    ...
    sections = CampusOracle::CampusData.get_instructing_sections('238382', @current_terms)
    sections.should_not be_nil
    sections.length.should == 2 if CampusOracle::CampusData.test_data?
  end

  it 'finds all active secondary sections for the course' do
    sections = CampusOracle::CampusData.get_course_secondary_sections(2013, 'D', 'BIOLOGY', '1A')
  ...
  it "should check whether the db is alive" do
    alive = CampusOracle::CampusData.database_alive?
    alive.should be_true
  end

  it "should report DB outage" do
    CampusOracle::CampusData.connection.stub(:select_one).and_raise(
  ...
  it "should include an ex_student role for a person with a STUDENT-STATUS-EXPIRED affiliation" do
    result = CampusOracle::CampusData.get_person_attributes( 238382 ) # Bernie!!
    result[:roles][:ex_student].should be_true
  end

  it "should include a guest role", if: CampusOracle::CampusData.test_data? do
    result = CampusOracle::CampusData.get_person_attributes(19999969)
    result[:roles][:guest].should be_true
  end

  it "should return class schedule data" do
    data = CampusOracle::CampusData.get_section_schedules("2013", "D", "16171")
  ...
  it "should return instructor data given a course control number" do
    data = CampusOracle::CampusData.get_section_instructors("2013", "D", "7309")
    data.should_not be_nil
    if CampusOracle::CampusData.test_data?
  ...
  it "should be able to get a whole lot of user records" do
    ...
    user_data = CampusOracle::CampusData.get_basic_people_attributes(lotsa_uids)
  ...
  it "should be able to look up Tammi's student info" do
    info = CampusOracle::CampusData.get_student_info "300939"
  ...
  it "should use affiliations to decide whether the user is a student" do
    CampusOracle::CampusData.is_student?(
        {
            'student_id' => 1,
  ...
  it "should find a grad student that used to be an undergrad", if: CampusOracle::CampusData.test_data? do
    CampusOracle::CampusData.is_previous_ugrad?("212388").should be_true
    CampusOracle::CampusData.is_previous_ugrad?("212389").should be_true #grad student expired, previous ugrad
    CampusOracle::CampusData.is_previous_ugrad?("212390").should be_false #grad student, but not previous ugrad
    CampusOracle::CampusData.is_previous_ugrad?("300939").should be_true #ugrad only
  end

  it "should say an instructor has instructional history", if: CampusOracle::CampusData.test_data? do
    CampusOracle::CampusData.has_instructor_history?("238382", Settings.sakai_proxy.academic_terms.instructor).should be_true
  end

  it "should say a student has student history", if: CampusOracle::CampusData.test_data? do
    CampusOracle::CampusData.has_student_history?("300939", Settings.sakai_proxy.academic_terms.student).should be_true
  end

  it "should say a staff member does not have instructional or student history", if: CampusOracle::CampusData.test_data? do
    CampusOracle::CampusData.has_instructor_history?("2040", Settings.sakai_proxy.academic_terms.instructor).should be_false
    CampusOracle::CampusData.has_student_history?("2040", Settings.sakai_proxy.academic_terms.student).should be_false
  end

  context "when searching for users by name" do
    it "should raise exception if search string argument is not a string" do
      expect { CampusOracle::CampusData.find_people_by_name(12345) }.to raise_error(ArgumentError, "Search text argument must be a string")
    end

    it "should raise exception if row limit argument is not an integer" do
      expect { CampusOracle::CampusData.find_people_by_name("TEST-", "15") }.to raise_error(ArgumentError, "Limit argument must be a Fixnum")
    end

    it "should escape user input to avoid SQL injection", :testext => true do
      CampusOracle::CampusData.connection.should_receive(:quote_string).with("anything' OR 'x'='x").and_return("anything'' OR ''x''=''x")
      user_data = CampusOracle::CampusData.find_people_by_name("anything' OR 'x'='x")
    end

    it "should be able to find users by last name separated by a comma and space", :testext => true do
      user_data = CampusOracle::CampusData.find_people_by_name("smith, j", 10)
    ...
    it "should be able to limit the number of results", :testext => true do
      user_data = CampusOracle::CampusData.find_people_by_name("smith", 2)
      expect(user_data).to be_an_instance_of Array
  ...
  context "when searching for users by email" do
    it "should raise exception if search string argument is not a string" do
      expect { CampusOracle::CampusData.find_people_by_email(12345) }.to raise_error(ArgumentError, "Search text argument must be a string")
    end

    it "should raise exception if row limit argument is not an integer" do
      expect { CampusOracle::CampusData.find_people_by_email("test-", "15") }.to raise_error(ArgumentError, "Limit argument must be a Fixnum")
    end

    it "should escape user input to avoid SQL injection", :testext => true do
      CampusOracle::CampusData.connection.should_receive(:quote_string).with("anything' OR 'x'='x").and_return("anything'' OR ''x''=''x")
      user_data = CampusOracle::CampusData.find_people_by_email("anything' OR 'x'='x")
    end

    it "should be able to find users by partial email", :testext => true do
      user_data = CampusOracle::CampusData.find_people_by_email("johnson")
      expect(user_data).to be_an_instance_of Array
      if user_data.count > 0
        expect(user_data[0]).to be_an_instance_of Hash
        expect(user_data[0]['first_name']).to be_an_instance_of String
    ...
    it "should provide row number and count column", :testext => true do
      user_data = CampusOracle::CampusData.find_people_by_email("john", 1)
      expect(user_data).to be_an_instance_of Array
      if user_data.count > 0
        expect(user_data[0]).to be_an_instance_of Hash
        expect(user_data[0]['row_number']).to be_an_instance_of String
    ...
    it "should be able to limit the number of results", :testext => true do
      user_data = CampusOracle::CampusData.find_people_by_email("john", 5)
      expect(user_data).to be_an_instance_of Array
  ...
  context "when searching for users by student id" do
    it "should raise exception if argument is not a string" do
      expect { CampusOracle::CampusData.find_people_by_student_id(12345) }.to raise_error(ArgumentError, "Argument must be a string")
    end

    it "should raise exception if argument is not an integer string" do
      expect { CampusOracle::CampusData.find_people_by_student_id('2890abc') }.to raise_error(ArgumentError, "Argument is not an integer string")
    end

    it "should not find results by partial student id", if: CampusOracle::CampusData.test_data? do
      user_data = CampusOracle::CampusData.find_people_by_student_id("8639")
      expect(user_data).to be_an_instance_of Array
      expect(user_data.count).to eq 0
    end

    it "should find results by exact student id", if: CampusOracle::CampusData.test_data? do
      user_data = CampusOracle::CampusData.find_people_by_student_id("863980")
      expect(user_data).to be_an_instance_of Array
    ...
    it "should include row number and count as 1", if: CampusOracle::CampusData.test_data? do
      user_data = CampusOracle::CampusData.find_people_by_student_id("863980")
      expect(user_data).to be_an_instance_of Array
      expect(user_data[0]).to be_an_instance_of Hash
  ...
  context "when searching for users by LDAP user id" do
    it "should raise exception if argument is not a string" do
      expect { CampusOracle::CampusData.find_people_by_uid(300847) }.to raise_error(ArgumentError, "Argument must be a string")
    end

    it "should raise exception if argument is not an integer string" do
      expect { CampusOracle::CampusData.find_people_by_uid('300abc') }.to raise_error(ArgumentError, "Argument is not an integer string")
    end

    it "should not find results by partial user id", if: CampusOracle::CampusData.test_data? do
      user_data = CampusOracle::CampusData.find_people_by_uid("3008")
      expect(user_data).to be_an_instance_of Array
      expect(user_data.count).to eq 0
    end

    it "should find user by exact LDAP user ID", if: CampusOracle::CampusData.test_data? do
      user_data = CampusOracle::CampusData.find_people_by_uid("300847")
      expect(user_data).to be_an_instance_of Array
      expect(user_data.count).to eq 1
      expect(user_data[0]).to be_an_instance_of Hash
  ...
  context "when checking integer format of string" do
    it "raises exception if argument is not a string" do
      expect { CampusOracle::CampusData.is_integer_string?(188902) }.to raise_error(ArgumentError, "Argument must be a string")
    end

    it "returns true if string is successfully converted to an integer" do
      expect(CampusOracle::CampusData.is_integer_string?("189023")).to be_true
    end

    it "returns false if string is not successfully converted to an integer" do
      expect(CampusOracle::CampusData.is_integer_string?("18dfsd9023")).to be_false
      expect(CampusOracle::CampusData.is_integer_string?("254AbCdE")).to be_false
      expect(CampusOracle::CampusData.is_integer_string?("98,()@")).to be_false
      expect(CampusOracle::CampusData.is_integer_string?("2390.023")).to be_false
    end
  end
