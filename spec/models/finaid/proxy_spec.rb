...
describe Proxy do
  ...
  let(:live_oski_finaid){ Proxy.new({user_id: "61889",  term_year: this_year }).get }
  let(:fake_oski_finaid){ Proxy.new({user_id: "61889",  term_year: this_year, fake: true}).get }
  let(:live_non_student){ Proxy.new({user_id: '212377', term_year: this_year}).get }
  ...
  context "fake finaid tests" do
    ...
    context "Test-Emeritus live feed with no data" do
      ...
      before(:each) { Proxy.any_instance.stub(:lookup_student_id).and_return(nil) }
