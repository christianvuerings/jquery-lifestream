...
describe "MyTasks" do
  before(:each) do
    ...
    @fake_google_tasks_list_proxy = Google::GoogleTasksListProxy.new({fake: true})
    @fake_google_update_task_proxy = Google::GoogleUpdateTaskProxy.new({fake: true})
    @fake_google_clear_completed_tasks_proxy = Google::GoogleClearTaskListProxy.new({fake: true})
    @fake_google_tasks_array = @fake_google_tasks_list_proxy.tasks_list
    @real_google_tasks_list_proxy = Google::GoogleTasksListProxy.new(:access_token => Settings.google_proxy.test_user_access_token,
                                                             :refresh_token => Settings.google_proxy.test_user_refresh_token,
                                                             :expiration_time => 0)
    @real_google_update_task_proxy = Google::GoogleUpdateTaskProxy.new(:access_token => Settings.google_proxy.test_user_access_token,
                                                               :refresh_token => Settings.google_proxy.test_user_refresh_token,
                                                               :expiration_time => 0)
    @real_google_clear_completed_tasks_proxy = Google::GoogleClearTaskListProxy.new(:access_token => Settings.google_proxy.test_user_access_token,
                                                                            :refresh_token => Settings.google_proxy.test_user_refresh_token,
                                                                            :expiration_time => 0)
  ...
  it "should load nicely with the pre-recorded fake Google and Canvas proxy feeds using the server's timezone" do
    ...
    begin
      ...
      Google::GoogleTasksListProxy.stub(:new).and_return(@fake_google_tasks_list_proxy)
      ...
      valid_feed["tasks"].each do |task|
        ...
        if task["emitter"] == Google::GoogleProxy::APP_ID
  ...
  it "should fail google update_tasks param validation, invalid parameter(s)" do
    ...
    my_tasks = MyTasks::Merged.new @user_id
    expect {
      my_tasks.update_task({"type" => "sometype", "emitter" => Google::GoogleProxy::APP_ID, "status" => "completed" })
    }.to raise_error { |error|
      error.should be_a(ArgumentError)
  ...
  it "should fail google update_tasks with unauthorized access" do
    Google::GoogleProxy.stub(:access_granted?).and_return(false)
    my_tasks = MyTasks::Merged.new @user_id
    response = my_tasks.update_task({"type" => "sometype", "emitter" => Google::GoogleProxy::APP_ID, "status" => "completed", "id" => "foo"})
    response.should == {}
  end
  ...
  it "should fail google update_tasks with a remote proxy error" do
    Google::GoogleProxy.stub(:access_granted?).and_return(true)
    Google::GoogleUpdateTaskProxy.stub(:new).and_return(@fake_google_update_task_proxy)
    my_tasks = MyTasks::Merged.new @user_id
  ...
  it "should succeed google update_tasks with a properly formatted params" do
    ...
    response = my_tasks.update_task({"title" => "some bogus title", "notes" => "some bogus notes", "type" => "sometype", "emitter" => Google::GoogleProxy::APP_ID, "status" => "completed", "id" => task_id}, task_list_id)
    response["type"].should == "task"
    response["id"].should == task_id
    response["emitter"].should == Google::GoogleProxy::APP_ID
  ...
  it "should invalidate merged cache and google tasks cache on an update_task for google" do
    ...
    response = my_tasks.update_task({"type" => "sometype", "emitter" => Google::GoogleProxy::APP_ID, "status" => "completed", "id" => task_id}, task_list_id)
  end
  ...
  it "should return Google tasks when Canvas service is unavailable" do
    ...
    tasks.each do |task|
      task["emitter"].should == Google::GoogleProxy::APP_ID
    end
  end

  it "should clear completed Google tasks" do
    Google::GoogleProxy.stub(:access_granted?).and_return(true)
    Google::GoogleClearTaskListProxy.stub(:new).and_return(@fake_google_clear_completed_tasks_proxy)
    my_tasks_model = MyTasks::Merged.new(@user_id)
  ...
  it "should simulate a non-responsive google", :testext => true do
    ...
    Google::GoogleTasksListProxy.stub(:new).and_return(@real_google_tasks_list_proxy)
    Google::GoogleUpdateTaskProxy.stub(:new).and_return(@real_google_update_task_proxy)
    Google::GoogleClearTaskListProxy.stub(:new).and_return(@real_google_clear_completed_tasks_proxy)
    my_tasks_model = MyTasks::Merged.new(@user_id)
    response = my_tasks_model.clear_completed_tasks params={"emitter" => "Google"}
    response.should == {:tasksCleared => false}
    response = my_tasks_model.update_task({"type" => "sometype", "emitter" => Google::GoogleProxy::APP_ID, "status" => "completed", "id" => "1"}, "1")
    response.should == {}
    valid_feed = my_tasks_model.get_feed
...
def get_task_list_id_and_task_id
  ...
  create_proxy = Google::GoogleCreateTaskListProxy.new(:fake => true)
  ...
  insert_proxy = Google::GoogleInsertTaskProxy.new(:fake => true)
