...
describe MyActivities::Canvas do
  ...
  before do
    ...
    @fake_activity_stream_proxy = Canvas::CanvasUserActivityStreamProxy.new({fake: true})
  ...
  it "should be able to process a normal canvas feed" do
    ...
    activities.each do | activity |
      ...
      activity[:emitter].should == Canvas::CanvasProxy::APP_NAME
  ...
  it "should merge raw Canvas feed with transformed site feeds" do
    ...
    Canvas::CanvasUserActivityStreamProxy.stub(:new).and_return(stub_proxy(:user_activity, active_stream_feed))
    canvas_sites = [
      {
        id: '1',
        name: 'Course Code 1',
  ...
  it "should be able to ignore malformed entries from the canvas feed" do
    ...
    Canvas::CanvasUserActivityStreamProxy.stub(:new).and_return(stub_proxy(:user_activity, flawed_activity_stream))
    activities = MyActivities::Canvas.get_feed(@user_id, [])
    activities.instance_of?(Array).should == true
  ...
  it "should sometimes have score and instructor message appended to the summary field" do
    ...
    Canvas::CanvasUserActivityStreamProxy.stub(:new).and_return(@fake_activity_stream_proxy)
    activities = MyActivities::Canvas.get_feed(@user_id, [])
