...
describe Bearfacts::BearfactsProfileProxy do

  it "should get Oski Bear's profile from fake vcr recordings" do
    client = Bearfacts::BearfactsProfileProxy.new({:user_id => "61889", :fake => true})
    xml = client.get
    xml.should_not be_nil
  end

  it "should fail gracefully on a user whose student_id can't be found" do
    client = Bearfacts::BearfactsProfileProxy.new({:user_id => "0", :fake => true})
    response = client.get
  ...
  it "should get Oski Bear's profile from a real server", :testext => true do
    client = Bearfacts::BearfactsProfileProxy.new({:user_id => "61889", :fake => false})
    xml = client.get
    xml.should_not be_nil
  end
  ...
  context "connection failure" do
    ...
    subject { Bearfacts::BearfactsProfileProxy.new({:user_id => "61889", :fake => false}).get }
