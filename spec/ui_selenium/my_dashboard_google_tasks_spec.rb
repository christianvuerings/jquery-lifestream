...
describe 'The My Dashboard task manager', :testui => true do
  ...
  if ENV["UI_TEST"]
    ...
    context 'for Google tasks' do
      ...
      before(:example) do
        @to_do_card.delete_all_tasks
      end

      context 'when adding a task' do

        it 'allows a user to create only one task at a time' do
          WebDriverUtils.wait_for_page_and_click @to_do_card.new_task_button_element
        ...
        it 'allows a user to cancel the creation of a new task' do
          ...
          WebDriverUtils.wait_for_element_and_click @to_do_card.cancel_new_task_button_element
        ...
        it 'allows a user to create a task without a note' do
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.today_task_one_edit_button_element
        ...
        it 'allows the user to show more overdue tasks in ascending date order' do
          (1..11).each do |i|
            ...
            @to_do_card.wait_until(task_wait) { @to_do_card.overdue_task_one_title == "task #{i.to_s}" }
            @to_do_card.wait_until(task_wait) { @to_do_card.overdue_task_one_date == WebDriverUtils.ui_numeric_date_format(date) }
        ...
        it 'allows the user to show more tasks due today in ascending creation sequence' do
          (1..11).each do |i|
            ...
            @to_do_card.wait_until(task_wait) { @to_do_card.last_today_task_title == "task #{i.to_s}" }
            @to_do_card.wait_until(task_wait) { @to_do_card.last_today_task_date == WebDriverUtils.ui_numeric_date_format(date) }
        ...
        it 'allows the user to show more tasks due in the future in ascending date order' do
          (1..11).each do |i|
            ...
            @to_do_card.wait_until(task_wait) { @to_do_card.last_future_task_title == "task #{i.to_s}" }
            @to_do_card.wait_until(task_wait) { @to_do_card.last_future_task_date == WebDriverUtils.ui_numeric_date_format(date) }
        ...
        it 'allows the user to show more unscheduled tasks in descending creation sequence' do
          (1..11).each do |i|
            ...
            @to_do_card.wait_until(task_wait) { @to_do_card.unsched_task_one_title == "task #{i.to_s}" }
            @to_do_card.wait_until(task_wait) { @to_do_card.unsched_task_one_date == WebDriverUtils.ui_numeric_date_format(today) }
      ...
      context 'when editing an existing task' do
        ...
        it 'allows a user to edit the title of an existing task' do
          ...
          @to_do_card.click_add_task_button
          WebDriverUtils.wait_for_page_and_click @to_do_card.scheduled_tasks_tab_element
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.today_task_one_save_button_element
          @to_do_card.wait_until(task_wait) { @to_do_card.today_task_one_title == 'Edited Title' }
        end
        ...
        it 'allows a user to make an unscheduled task overdue' do
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.unsched_task_one_toggle_element
          WebDriverUtils.wait_for_page_and_click @to_do_card.unsched_task_one_edit_button_element
          @to_do_card.edit_unsched_task_one(nil, WebDriverUtils.ui_date_input_format(yesterday), nil)
          WebDriverUtils.wait_for_page_and_click @to_do_card.unsched_task_one_save_button_element
        ...
        it 'allows a user to make an overdue task unscheduled' do
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.overdue_task_one_edit_button_element
          @to_do_card.edit_overdue_task_one(nil, '', nil)
          WebDriverUtils.wait_for_page_and_click @to_do_card.overdue_task_one_save_button_element
        ...
        it 'allows a user to edit notes on an existing task' do
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.unsched_task_one_toggle_element
          @to_do_card.unsched_task_one_notes_element.when_visible(timeout=task_wait)
          @to_do_card.wait_until(task_wait) { @to_do_card.unsched_task_one_notes == 'The edited note for this task' }
        end
        ...
        it 'allows a user to remove notes from an existing task' do
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.unsched_task_one_toggle_element
          @to_do_card.unsched_task_one_notes_element.when_visible(timeout=task_wait)
          @to_do_card.wait_until(task_wait) { @to_do_card.unsched_task_one_notes == '' }
        end
        ...
        it 'allows a user to edit multiple scheduled tasks at once' do
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.future_task_one_edit_button_element
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.overdue_task_one_save_button_element
          WebDriverUtils.wait_for_page_and_click @to_do_card.future_task_one_save_button_element
          WebDriverUtils.wait_for_page_and_click @to_do_card.today_task_one_save_button_element
          WebDriverUtils.wait_for_page_and_click @to_do_card.overdue_task_one_toggle_element
          @to_do_card.wait_until(task_wait) { @to_do_card.overdue_task_one_title == 'Overdue task edited' }
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.today_task_one_toggle_element
          @to_do_card.wait_until(task_wait) { @to_do_card.today_task_one_title == 'Today task edited' }
          expect(@to_do_card.today_task_one_date).to eql(WebDriverUtils.ui_numeric_date_format(today))
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.future_task_one_toggle_element
          @to_do_card.wait_until(task_wait) { @to_do_card.future_task_one_title == 'Future task edited' }
        ...
        it 'allows a user to cancel the edit of an existing task' do
          ...
          WebDriverUtils.wait_for_page_and_click @to_do_card.today_task_one_cancel_button_element
          WebDriverUtils.wait_for_page_and_click @to_do_card.today_task_one_toggle_element
          @to_do_card.today_task_one_notes_element.when_visible(timeout=task_wait)
      ...
      context 'when completing tasks' do
        ...
        it 'allows the user to show more completed tasks sorted first by descending task date and then by descending task creation date' do
          ...
          (1..3).each do |i|
            ...
            @to_do_card.click_add_task_button
            @to_do_card.wait_until(task_wait) { @to_do_card.overdue_task_one_title == "overdue task #{i.to_s}" }
          ...
          (1..3).each do |i|
            ...
            @to_do_card.click_add_task_button
            @to_do_card.wait_until(task_wait) { @to_do_card.today_task_one_title == "today task #{i.to_s}" }
          ...
          (1..3).each do |i|
            ...
            @to_do_card.click_add_task_button
            @to_do_card.wait_until(task_wait) { @to_do_card.future_task_one_title == "future task #{i.to_s}" }
          ...
          (1..3).each do |i|
            ...
            @to_do_card.click_add_task_button
            @to_do_card.wait_until(task_wait) { @to_do_card.unsched_task_one_title == "unscheduled task #{i.to_s}" }
            expected_task_titles.push(@to_do_card.unsched_task_one_title)
            @to_do_card.complete_unsched_task_one
          end
          WebDriverUtils.wait_for_page_and_click @to_do_card.completed_tasks_tab_element
