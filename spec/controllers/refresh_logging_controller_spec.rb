...
describe RefreshLoggingController do
  ...
  it "should not allow non-admin users to do anything to the logging level" do
    ...
    User::UserAuth.stub(:where).and_return([User::UserAuth.new(uid: @user_id, is_superuser: false, active: true)])
  ...
  it "should not attempt to change log settings to a new bad config value" do
    session[:user_id] = @user_id
    Rails.env.stub(:production?).and_return(true)
    User::UserAuth.stub(:where).and_return([User::UserAuth.new(uid: @user_id, is_superuser: true, active: true)])
