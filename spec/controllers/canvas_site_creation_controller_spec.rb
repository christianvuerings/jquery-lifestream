describe CanvasSiteCreationController do
  ...
  describe '#authorizations' do
    before do
      session['user_id'] = uid
      allow_any_instance_of(CanvasLti::SiteCreation).to receive(:authorizations).and_return(authorizations_hash)
    end

    it_should_behave_like 'an api endpoint' do
      before { allow_any_instance_of(CanvasLti::SiteCreation).to receive(:authorizations).and_raise(RuntimeError, 'Something went wrong') }
