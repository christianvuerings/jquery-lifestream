module FakeableProxy
  ...
  if Settings.application.fake_proxies_enabled
    VCR.configure do |c|
      ...
      c.debug_logger = File.open("#{CalcentralLogging.log_root}/vcr-debug.log", 'w') unless Rails.env.production?
  ...
  def wrap_request(proxy_id, force_fake = nil, extra_cassette_options = {}, &proc_block)
    ...
    return yield unless Settings.application.fake_proxies_enabled && (force_fake || Settings.freshen_vcr)
