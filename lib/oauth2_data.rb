class Oauth2Data < ActiveRecord::Base
  ...
  def self.get_canvas_email(user_id)
    get_appdata_field(Canvas::CanvasProxy::APP_ID, user_id, 'email')
  end
  ...
  def self.update_canvas_email!(user_id)
    ...
    use_pooled_connection {
      authenticated_entry = self.where(uid: user_id, app_id: Canvas::CanvasProxy::APP_ID).first
      return unless authenticated_entry
      userinfo = Canvas::CanvasUserProfileProxy.new(user_id: user_id).user_profile
