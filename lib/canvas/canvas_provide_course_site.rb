class CanvasProvideCourseSite < CanvasCsv
  ...
  def import_course_site(canvas_course_row)
    ...
    response = Canvas::CanvasSisImportProxy.new.import_courses(@import_data['courses_csv_file'])
  ...
  def import_sections(canvas_section_rows)
    ...
    response = Canvas::CanvasSisImportProxy.new.import_sections(@import_data['sections_csv_file'])
    if response.blank?
  ...
  def import_users(canvas_user_rows)
    ...
    response = Canvas::CanvasSisImportProxy.new.import_users(@import_data['users_csv_file'])
    if response.blank?
  ...
  def import_enrollments(canvas_enrollment_rows)
    ...
    response = Canvas::CanvasSisImportProxy.new.import_enrollments(@import_data['enrollments_csv_file'])
    if response.blank?
  ...
  def expire_instructor_sites_cache
    Canvas::CanvasUserCoursesProxy.expire(@uid)
  ...
  def generate_course_site_definition(term_yr, term_cd, subaccount, campus_course_data)
    if (sis_id = generate_unique_sis_course_id(Canvas::CanvasExistenceCheckProxy.new, campus_course_data[:slug], term_yr, term_cd))
      {
        'course_id' => sis_id,
        'short_name' => campus_course_data[:course_number],
        'long_name' => campus_course_data[:title],
        'account_id' => subaccount,
        'term_id' => Canvas::CanvasProxy.term_to_sis_id(term_yr, term_cd),
        'status' => 'active'
      }
    else
  ...
  def generate_section_definitions(term_yr, term_cd, sis_course_id, campus_section_data)
    ...
    sections = []
    existence_proxy = Canvas::CanvasExistenceCheckProxy.new
  ...
  def subaccount_for_department(department)
    ...
    if !Canvas::CanvasExistenceCheckProxy.new.account_defined?(subaccount)
