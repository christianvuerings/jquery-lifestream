class CanvasIncrementalEnrollments < CanvasCsv
  ...
  def self.canvas_section_enrollments(canvas_section_id)
    ...
    canvas_section_enrollments = Canvas::CanvasSectionEnrollmentsProxy.new(:section_id => canvas_section_id).list_enrollments
  ...
  def refresh_existing_term_sections(term, enrollments_csv, known_users, users_csv)
    canvas_sections_csv = Canvas::CanvasSectionsReportProxy.new.get_csv(term)
    ...
    canvas_sections_csv.each do |canvas_section|
      if (section_id = canvas_section['section_id'])
        if (course_id = canvas_section['course_id'])
          ...
          if (campus_section = Canvas::CanvasProxy.sis_section_id_to_ccn_and_term(section_id))
